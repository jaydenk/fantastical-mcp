...
However, Fantastical doesn't always add the original date to the master's EXDATE list when a move happens over Exchange, which would otherwise produce a "ghost" occurrence at the original time *plus* the moved event at the new time. To prevent this, Pass 2 joins each master to its sibling detached rows and feeds their `recurrenceInstanceDate` values in as additional EXDATEs during expansion.

Linking master to sibling is subtle on **Exchange/Outlook** calendars: a detached occurrence does **not** share a raw `exchangeUID` with its master. An Exchange `GlobalObjectId` (GOID) encodes the *original occurrence date* into bytes 16–19, so every moved instance carries a different `exchangeUID` string. The series identity is the `CleanGlobalObjectId` -- the same GOID with those 4 recurrence-id bytes zeroed -- whose trailing per-series Data GUID is shared across the master and all its exceptions. Pass 2 therefore narrows candidates with a cheap secondary-index match on that shared trailing GUID, then confirms each with a normalised `CleanGlobalObjectId` comparison before decoding any blob. Series without an `exchangeUID` (some CalDAV / local calendars) rely purely on the master's own EXDATE list, which is the convention those calendar providers follow reliably.
